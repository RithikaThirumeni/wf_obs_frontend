...
export default function TransactionsTable({translist}) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - translist.length) : 0;
  ...
  return (
    <React.Fragment>            
        ...
        <TableBody>
        {Array.isArray(translist)?(rowsPerPage > 0
            ? translist.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : translist
        ).map((item) => (
            <TableRow>
